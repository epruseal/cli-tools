...
def test_plain_number_parses() -> None:
    assert parse_article_query("839") == _article_no("839")


def test_with_jo_suffix() -> None:
    assert parse_article_query("제839조") == _article_no("839")
    assert parse_article_query("839조") == _article_no("839")


def test_whitespace_tolerated() -> None:
    assert parse_article_query("  제 839 조 의 2  ") == _article_no("839", "2")
